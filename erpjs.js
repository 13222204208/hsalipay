var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var f = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="edit" class="ui-btn">保存</a><a class="ui-btn" id="audit">审核</a>',
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSo/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印合同</a>',
				i = "",
				j = "",
				k = "",
				l = '',
			...
			billRequiredCheck ? (i = '', j = '',console.log('不是编辑')) : (g = l + g, this.$_checkName.parent().hide(),console.log('编辑'));
			var o = '';
		...
		loadGrid: function(a) {
			...
			function p(a) {
				...
				if (b) {
					...
					var f = {
						...
						safeDays: b.safeDays,
						used: b.used,
						tpno:'<a class="dotp" style="color:#9e9d9d" data-id="' + b.id + '" title="点击选择托盘" href="javascript:;">点击选择托盘</a>',
					   // tpdata:''
			...
			function amore(a,b,c){
			    ...
			    if(a){
			        if(a=='tpno'){
			             d='<a class="dotp" style="color:#9e9d9d" data-id="' + c.invId + '" title="点击选择托盘" href="javascript:;">点击选择托盘</a>'
			        }else{
			            d=a;
			        }
    					...
    					data: {
    						...
    						callback: function(a, b, c) {
    						   ...
    						   for (v in c) {
    						   	    ...
    						   	    "recovery":c[v].recovery=='是'||c[v].recovery=="1"?1:0,
    						   	    "tpplace":c[v].tpplace,
    						   	    'ininvid':c[v].ininvid,
    						   	        
    						   	    }
    						   	    if(c[v].recovery=='是'||c[v].recovery=="1"){
    						   	        recoveryQty++
			...
			for (var a = [], b = $("#grid").jqGrid("getDataIDs"), c = 0, d = b.length; d > c; c++) {
				...
				if ("" !== g.goods) {
					var h = $("#" + f).data("goodsInfo"),
						...
						console.log(g.qty)//输入数量
						console.log(g.used)//可用数量
